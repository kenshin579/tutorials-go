...
본 프로젝트는 WebRTC 스터디의 최종 실습 과제로,
Go(Echo) 기반 Signaling 서버와 React 18 프론트엔드를 사용하여
맥북 내장 카메라를 활용한 **1:1 화상 통화 애플리케이션**을 구현한다.

**목적:**
- WebRTC 핵심 개념을 실제 코드로 연결
- 실무에서 사용 가능한 구조(Echo + React) 경험
- Claude Code를 활용한 설계 → 구현 흐름 자동화
...
- **Backend**: Go + Echo (WebSocket 기반 Signaling Server)
- **Media**: 맥북 내장 카메라 및 마이크, WebRTC P2P 전송
...
### 5.3 Media Stream
- 브라우저에서 `getUserMedia()`를 사용해 video/audio 스트림을 획득한다
...
### 구조 요구사항
- WebRTC 관련 로직은 Custom Hook으로 분리한다
  - `useWebRTC`: WebRTC 연결 관리
  - `useSignaling`: WebSocket 시그널링 관리
...
- Media 처리는 하지 않음 (P2P로 직접 전송)

### API
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/ws?roomId={roomId}` | WebSocket 업그레이드, Signaling 통신 채널 |
