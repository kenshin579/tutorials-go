...
기존 글 스타일(`# 1.제목` / `## 1.1 소제목` 번호 체계) 준수. 글 흐름이 코드 폴더 구성(①→②→③→④)과 1:1 대응.

- 서론 — `time.Now()` 직접 호출의 문제 제기, 참고 자료 블록
- 1. 시간 의존 코드가 테스트하기 어려운 이유
  - 1.1 흔한 안티패턴 — 테스트 내 `time.Sleep` 대기, flaky 테스트
  - 1.2 해결의 공통 원리 — 시간을 코드 밖에서 주입받기
- 2. 패턴 ①: 시간을 파라미터로 받기
  - 2.1 쿠폰 만료 검사 예제 (`IsExpiredAt(now)`)
  - 2.2 테이블 드리븐 테스트로 경계값 검증
  - 2.3 한계 — 깊은 콜스택이면 매번 시간 전달 필요
- 3. 패턴 ②: nowFunc 필드 주입
  - 3.1 `OrderService`에 `nowFunc func() time.Time` 필드 (기본값 `time.Now`)
  - 3.2 테스트에서 고정 시간 주입
...
- 4. 패턴 ③: Clock 인터페이스 주입 (clockwork)
  ...
  - 4.3 언제 인터페이스까지 필요한가 (Ticker/Timer/Sleep 제어)
- 5. 패턴 ④: testing/synctest (Go 1.25+)
  - 5.1 코드 수정 없이 가상 시간 버블에서 테스트
  - 5.2 같은 TTL 캐시를 synctest로 테스트 — 패턴 ③과 비교
  - 5.3 제약 사항 (동시성+시간 특화, Go 1.25+ 필요)
- 6. 어떤 패턴을 언제 쓸까 — 선택 가이드 표 (실무 80%는 패턴 ②로 충분)
- 마무리 — 요약 + tutorials-go 예제 저장소 링크
